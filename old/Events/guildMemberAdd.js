...
module.exports = async(bot, member) => {
    ...
    db.query(`SELECT * FROM server WHERE guild = '${member.guild.id}'`, async (err, req) => {
        ...
        if((req[0].antiraid === "true")) {
            ...
            try {await member.user.send("Vous ne pouvez pas rejoindre se serveur car il est en mode antiraid !")} catch(err) {}
        ...
        let msg = await channel.send({content : `${member}, vous avez 2 minutes pour compléter le captcha ! si vous echouez vous serez exclue du serveur !`, files: [new Discord.AttachmentBuilder((await captcha.canvas).toBuffer(), {name: "captcha.png"})]})
        ...
        try {
            ...
            await msg.delete()
            try {await member.user.send("Vous avez mis trop de temps pour completer le captcha !")} catch (err) {}
            await channel.permissionOverwrites.delete(member.user.id)
